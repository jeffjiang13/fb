...
function CreateGroup({ setShowCreateChat, showCreateChat, type, chat }) {
  ...
  const createGroupValidation = Yup.object({
    chatName:
      type === "create" || type === "rename"
        ? Yup.string().required("Chat name is required.").min(3).max(40)
        : Yup.string(),
  });

  const input = useRef(null);
  const popUpRef = useRef();
  useOnClickOutside(popUpRef, showCreateChat, () => {
  ...
  useEffect(() => {
    console.log(CreateChatGroupData);
    if (type === "create" && CreateChatGroupData?.status === "success") {
      navigate(`/messages/${CreateChatGroupData.data.chat._id}`);
      setShowCreateChat(false);
          // Refresh if the chat is private
    if (CreateChatGroupData.data.chat.type === 'private') {
      ...
      <Card className={styles.card} innerRef={popUpRef}>
        ...
        <div className={styles.content}>
          <Formik
            ...
            onSubmit={(values) => {
              if (members.length < 1 && type === "create") {
                toast.error("Please add at least 1 members to chat");
                return;
              ...
              <Form className={styles.form} noValidate>
                <FormLoader loading={isLoading}>
                  ...
                  {(type === "create" || type === "add") && (
                    <p className={styles.label}>Add member to chat</p>
                  )}
                  {members.length > 0 && (
